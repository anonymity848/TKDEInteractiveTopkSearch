...
int PreferenceLearning(point_set *original_set, point_t *u, int k, int s, int error)
{
    ...
    while (accuracy < 1 - EQN_EPS && de_accuracy > 0)
    {
        numOfQuestion++;
        ...
        int compareResult = u->compare(p, q, error);
        if (compareResult == 1)
        {
            for (int i = 0; i < dim; i++)
            {
    ...
    original_set->findTopk(estimate_u, k, top_current);
    //top_current->printResult("PrefLearning", numOfQuestion, s, t1, 0, 0);
    ...
    double acc1 = testsum / groudtruthsum > 1? 1: testsum / groudtruthsum;

    top_current->printResult("PrefLearning", numOfQuestion, s, t1, 0, acc1);
