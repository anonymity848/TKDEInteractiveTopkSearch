...
void qh_initbuild( void) {
  ...
  else
  {
	  ...
	  //qh_printvertices(stdout, "", vertices);
...
vertexT *my_qh_makenewfacets(pointT *point /*visible_list*/) {
	facetT *visible, *newfacet = NULL, *newfacet2 = NULL, *neighbor, **neighborp;
	vertexT *apex;
	int numnew = 0;
	...
	if (visible->ridges) {
		visible->visitid = qh visit_id;
		/*May have problem*/
		newfacet2 = my_qh_makenew_nonsimplicial(visible, apex, &numnew);
	}
	if (visible->simplicial)
		newfacet = my_qh_makenew_simplicial(visible, apex, &numnew);
	//if (!qh ONLYgood) {
	//	if (newfacet2)  /* newfacet is null if all ridges defined */
	//		newfacet = newfacet2;
	//	if (newfacet)
